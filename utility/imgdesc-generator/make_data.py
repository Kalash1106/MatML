...
def prompt_gemini(img: Image, model: genai.GenerativeModel) -> str:
    """Construct a prompt and query Google Gemini

    ----------
    Parameters
    ----------
    :param img: an instance of PIL.Image
    :returns: A string containting LLM response
    """
    ...
    # Gemini might return the text in parts, combine them
...
def prompt_haiku(b64img: str, format: str, client: anthropic.Anthropic) -> str:
    """Construct a prompt and query Anthropic Claude 3 Haiku

    ---------
    Arguments
    ---------
    :params b64img: a string which contains the image in base64 encoded format
    :params format: the file extension of the image, i.e. jpeg, png
    :params client: an instance of anthropic.Anthropic

    :returns: A string containing LLM response
    """
...
def main():
    ...
    # we use a SQLite database initially for storing the responses
